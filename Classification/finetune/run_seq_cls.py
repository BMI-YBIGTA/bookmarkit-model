...
from processor import NsmcProcessor
...
def train(args,
    ...
    scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=int(
        t_total * args.warmup_proportion), num_training_steps=t_total)
    ...
    for epoch in mb:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.model_type not in ["distilkobert", "xlm-roberta"]:
                # Distilkobert, XLM-Roberta don't use segment_ids
...
def evaluate(args, model, eval_dataset, mode, global_step=None):
    ...
    output_eval_file = os.path.join(
        output_dir, "{}-{}.txt".format(mode, global_step) if global_step else "{}.txt".format(mode))
...
def main(cli_args):
    ...
    label_num = cli_args.label_num

    processor = NsmcProcessor(args, label_num)
    labels = processor.get_labels()  # 여기 바꾸기
    ...
    train_dataset = load_and_cache_examples(
        label_num, args, tokenizer, mode="train") if args.train_file else None
    dev_dataset = load_and_cache_examples(
        label_num, args, tokenizer, mode="dev") if args.dev_file else None
    test_dataset = load_and_cache_examples(
        label_num, args, tokenizer, mode="test") if args.test_file else None

    if dev_dataset == None:
        # If there is no dev dataset, only use testset
        args.evaluate_test_during_training = True

    if args.do_train:
        global_step, tr_loss = train(
            args, model, train_dataset, dev_dataset, test_dataset)
    ...
    if args.do_eval:
        ...
        for checkpoint in checkpoints:
            ...
            result = evaluate(args, model, test_dataset,
                              mode="test", global_step=global_step)
...
if __name__ == '__main__':
    ...
    cli_parser.add_argument("--label_num", type=int, required=True)
